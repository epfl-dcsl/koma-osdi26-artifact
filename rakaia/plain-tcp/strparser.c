                       ...
                       size_t max_msg_size, long timeo)
{
    ...
    while (eaten < orig_len) {
        ...
        if (!stm->strp.full_len) {
            ssize_t len;
            *((int *)head->cb) = stm->strp.offset;
            len = (*strp->cb.parse_msg)(strp, head);
            // pr_info(
            //     "[strp] the address of head is %p, eaten bytes is %d, length of msg is %d, full length is %d\n",
            //     head, stm->strp.offset , len, orig_len);
            // pr_info("[strp] the first byte is 0x%x\n", *head->data);
        ...
        //    pr_info("[strp] strp.full_len decided already\n"); 
        }
...
void strp_data_ready(struct strparser *strp)
{
    ...
    if (strp_read_sock(strp) == -ENOMEM){
        pr_info("strp_read_sock returns ENOMEM!\n");
        queue_work(strp_wq, &strp->work);
    }
}
