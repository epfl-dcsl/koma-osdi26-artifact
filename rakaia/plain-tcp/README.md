...
This directory contains the Rakaia kernel module source for the **plain-TCP**
configuration. Rakaia is a network-stack architecture that enables message
...
[`../patches/`](../patches/) — `rakaia.patch` adds
`AF_RAKAIA`/`PF_RAKAIA`/`SOL_RAKAIA` plus the hooks in `tcp.c`, `tcp_output.c`,
...
patched `6.8.0-rakaia` kernel produced by `bench/fabs/run_all.sh --patch-kernel`.

```bash
cd rakaia/plain-tcp
make
ls rakaia.ko    # produced here
```

The harness loads/unloads the module automatically per experiment via
`insmod ./rakaia.ko` / `rmmod rakaia`; manual usage is the same.
...
produced by `make` here is loaded by the harness whenever a `rakaia`-family
server config (`rakaia`, `silo-rakaia`, …) is selected. See
