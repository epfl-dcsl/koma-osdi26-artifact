...
static void send_pingack(struct rakaia_psock *psock, struct sk_buff *cloned_skb)
{
    const u8 *p;
    u8 buf[8];
    ...
    if (unlikely(skb_tailroom(skb) < 17)) {
        pr_err("[rakaia] PINGACK: insufficient tailroom %d\n", skb_tailroom(skb));
        __kfree_skb(skb);
        return;
    }
...
/* Create a small linear skb with TCP headroom and copy ctrl bytes in. */
static void send_window_update(struct rakaia_psock *psock, int inc)
{
    struct sk_buff *skb;
    struct sock *csk = psock->sk;
    // int mss_now = 0, size_goal;
...
/* Create a small linear skb with TCP headroom and copy ctrl bytes in. */
static void send_settings_ack(struct rakaia_psock *psock)
{
    struct sk_buff *skb;
    struct sock *csk = psock->sk;
    u8 *d;
    ...
    if (unlikely(skb_tailroom(skb) < 9)) {
        pr_err("[rakaia] SETTINGS ACK: insufficient tailroom %d\n",
               skb_tailroom(skb));
        __kfree_skb(skb);
        return;
    }
...
static void process_incoming_window_update(struct rakaia_psock *psock,
              ...
              const struct strp_callbacks *cb)
{
    ...
    strp->http2 = 1; // default to http2 for rakaia
