...
static int copy_rakaia_attach_from_user(struct rakaia_attach_user *info,
                                      void __user *arg)
{
    struct rakaia_attach_user_legacy legacy;
    ...
    if (legacy.bpf_fd != RAKAIA_ATTACH_EXTENDED_BPF_FD)
        return 0;
...
static void rakaia_free_strp_chain(struct sk_buff *head)
{
...
static void skb_queue_purge_rakaia(struct sk_buff_head *list)
{
    struct sk_buff *skb, *iter, *next;
    while ((skb = __skb_dequeue(list)) != NULL) {
            rakaia_free_strp_chain(skb);
    }
}
...
static int rakaia_queue_rcv_skb(struct rakaia_sock *ksk, struct sk_buff *skb)
{
    ...
    // pr_info("[strp] rakaia_queue_rcv_skb %p, strp_next %p\n", skb,
    // strp_next(skb));
    skb->sk = &ksk->sk;
...
static void rakaia_rcv_strparser(struct strparser *strp, struct sk_buff *skb)
{
    ...
    // pr_info("[strp] rakaia_rcv_strparser %p, strp_next %p\n", skb,
    // strp_next(skb));
                        ...
                        int flags)
{
    ...
    pr_info_log("[recvmsg] enter rakaia_recvmsg\n");
    ...
    // pr_info("[rakaia_recvmsg] the remote port and addresses are %04X %08X!:\n",
                       ...
                       int initial_conn_window)
{
    ...
    struct sock *csk;
    static const struct strp_callbacks cb = { .rcv_msg = rakaia_rcv_strparser,
    ...
    // not manipulate sk_write_space because we do not use rakaia tx.
...
static int rakaia_attach_ioctl(struct socket *sock, struct rakaia_attach_user *info)
{
    struct socket *csock;
    int err;
    ...
    err = rakaia_attach(sock, csock, info->initial_conn_window);
//     if (err) {
//         goto out;
//     }
...
static int rakaia_ioctl(struct socket *sock, unsigned int cmd, unsigned long arg)
{
    ...
    case SIOCRAKAIAATTACH: {
        struct rakaia_attach_user info;

        err = copy_rakaia_attach_from_user(&info, (void __user *)arg);
        if (err)
            return err;
...
static int rakaia_release(struct socket *sock)
{
     ...
     */
    skb_queue_purge_rakaia(&sk->sk_receive_queue);
    ...
    if (cnt == 0) {
        ...
        spin_lock_bh(&rakaia->lock);
        // skb_queue_purge_rakaia(&rakaia->all_msgs);
        spin_unlock_bh(&rakaia->lock);
    }
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("RAKAIA sockets");
