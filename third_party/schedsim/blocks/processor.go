...
type RTCProcessor struct {
	genericProcessor
	scale float64
	Index int // for ConnMultiQueue only
}
...
// processor which has access to many connections (connection-based queues), and
// picked the first queued message which is unbounded
...
func (p *ConnProcessor) Run() {
	for {
		req, hItem := p.ReadConnHeap(p.ConnHeapPtr, p.Index)
		t := req.GetServiceTime() + p.ctxCost
		p.Wait(t)
		// Get the initTime of the first item in the connection queue
		// back to the min-heap
		nreq := p.PeekInqueueI(hItem.ConnID)
		if nreq != nil {
			preq := nreq.(*ConnReq)
			hItem.InitTime = preq.GetInitTime()
			heap.Push(p.ConnHeapPtr, hItem)
		}
		// TODO: figure out if getting the first queue makes sense?
		p.SetInQueueinProcess(hItem.ConnID, false)
		if monitorReq, ok := req.(*MonitorReq); ok {
			monitorReq.finalLength = p.GetInQueueLen(0)
		}
		p.reqDrain.TerminateReq(req)
