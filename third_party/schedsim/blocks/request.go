...
func (r Request) GetDelay() float64 {
	// fmt.Printf("Latency of Request: %f, (%f - %f)\n", engine.GetTime()-r.InitTime, engine.GetTime(), r.InitTime)
...
type ConnReqCreator struct {
	ConnNum int
}

func (rc ConnReqCreator) NewRequest(serviceTime float64) engine.ReqInterface {
	// Note that the connection id is generated later, according to the actual desired number of connections, here we have it as 0 first
	return &ConnReq{Request{InitTime: engine.GetTime(), ServiceTime: serviceTime}, 0}
}
