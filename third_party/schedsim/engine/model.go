...
type model struct {
	...
	bookkeeping       []Stats
	ConnHeapPtr       *ConnHeap
	blockedProcessors *list.List
}
...
func (m *model) registerBlockEvent(e blockEventInterface) {
	if e.getQueues() == nil {
		// ConnHeap Block Event
		m.blockedProcessors.PushBack(e)
		return
	}
...
func (m *model) run(threshold float64) {
	...
	for m.time < threshold {
		// Note that m.queues here are outqueues of the request generator.
...
func (m *model) ConnHeapRun(threshold float64) {
	// wait for all actors to start and add an event or block on a queue
	for i := 0; i < m.actorCount; i++ {
		m.waitActor()
	}
	...
	for m.time < threshold {
		// Note that m.queues here are outqueues of the request generator.
		if m.ConnHeapPtr.Len() > 0 {
			for e := m.blockedProcessors.Front(); e != nil && m.ConnHeapPtr.Len() > 0; e = e.Next() {
				be := e.Value.(blockEventInterface)
				be.getChannel() <- 1
				m.blockedProcessors.Remove(e)
				m.waitActor()
			}
		}
...
func RegisterConnHeap(h *ConnHeap) {
	mdl.ConnHeapPtr = h
}
...
func ConnHeapRun(threshold float64) {
	mdl.ConnHeapRun(threshold)
}
