...
func (a *Actor) GetInQueueinProcess(idx int) bool {
	return a.inQueues[idx].GetinProcess()
}

func (a *Actor) SetInQueueinProcess(idx int, v bool) {
	a.inQueues[idx].SetinProcess(v)
}

func (a *Actor) GetOutQueueinProcess(idx int) bool {
	return a.outQueues[idx].GetinProcess()
}

func (a *Actor) SetOutQueueinProcess(idx int, v bool) {
	a.outQueues[idx].SetinProcess(v)
}
...
func (a *Actor) ReadInQueueI_ConnHeap(idx int) ReqInterface {
	if a.inQueues[idx].Len() > 0 {
		a.SetInQueueinProcess(idx, true)
		return a.inQueues[idx].Dequeue()
	}
	bEvent := blockEvent{wakeUpCh: a.wakeUpCh, queues: nil}
	a.toModel <- bEvent
	<-a.wakeUpCh
	return a.ReadInQueueI(idx)
}

func (a *Actor) PeekInqueueI(idx int) ReqInterface {
	if a.inQueues[idx].Len() > 0 {
		return a.inQueues[idx].Peekqueue()
	}
	return nil
}

func (a *Actor) GetInQueue() []QueueInterface {
	return a.inQueues
}

func (a *Actor) GetWakUpCh() chan int {
	return a.wakeUpCh
}

func (a *Actor) ReadConnHeap(h *ConnHeap, pindex int) (ReqInterface, *ConnHeapItem) {
	// h.PrintAllElements()
	hItem := h.SafePop()
	if hItem != nil {
		return a.ReadInQueueI_ConnHeap(hItem.ConnID), hItem
	}
	// block
	bEvent := blockEvent{wakeUpCh: a.wakeUpCh, queues: nil}
	a.toModel <- bEvent
	<-a.wakeUpCh
	return a.ReadConnHeap(h, pindex)
}
