...
First, we need to install the dependencies of lancet:
```
sudo apt install -y python3-pip libssl-dev python3-virtualenv
pip3 install cmake
python3 -m pip install --user virtualenv
```

Then, we need to install golang and grpc:
For golang:
```
cd ~
wget https://go.dev/dl/go1.21.0.linux-amd64.tar.gz
sudo rm -rf /usr/local/go && sudo rm -rf /usr/local/go && sudo tar -C /usr/local -xzf go1.21.0.linux-amd64.tar.gz
...
git clone --recurse-submodules -b v1.60.1 --depth 1 --shallow-submodules https://github.com/grpc/grpc
...
cd grpc 
sudo test/distrib/cpp/run_distrib_test_cmake_module_install.sh
```
...
```
If the host is only a localhost, simply run
```
make deploy HOSTS=localhost
```
...
Usage of ./coordinator/coordinator:
  ...
  -appProto string
    	application protocol (default "echo:1028")
  -ciSize int
...
```
-appProto <memcache-bin|memcache-id|memcache-ascii|redis>_<key_size_random_generator>_<value_size_random_generator>_<key_count>_<read_write_ratio>_<key_selector(rr|uni)>
```
