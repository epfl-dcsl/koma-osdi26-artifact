...
static __thread struct rakaia_msg_ctx msg_ctx;
...
static int handle_ret(int fd, ssize_t ret, int line) {
  // printf("RAKAIA recvmsg does not return 0, handle ret %zd !\n", ret);
  if (ret == 0) {
    close(fd);
...
static void *tcp_thread_main(void *arg) {
  ...
  while (1) {
    ...
    assert(nfds > 0);
    // printf("Get new connection or request %d\n", thread_no);
    ...
    for (i = 0; i < nfds; i++) {
      if (events[i].data.u32 == 0) {
        ...
        // rakaia_attach(rakaia_fd0, conn_sock, bpf_prog_fd);
        printf("Attach rakaia to new connection %d\n", conn_sock);

        // Perform the TLS handshake (in user space)
        if (SSL_accept(ssl) <= 0)
        {
          ERR_print_errors_fp(stderr);
        }
        printf("Successfully did handshake%d\n", conn_sock);
...
void start_rakaia_server(const char *str) {
