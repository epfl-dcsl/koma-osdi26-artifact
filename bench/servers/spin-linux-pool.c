...
static void help(const char *prgname) {
  printf("Usage: %s [--udp] service-time-distribution service-protocol [io-threads] [worker-threads]\n"
         "\n"
         ...
         "\n"
         "io-threads sets the number of pool I/O threads. If omitted, the "
         "server default is used.\n"
         "worker-threads sets the number of pool worker threads. If omitted, "
...
int main(int argc, char *argv[]) {
  ...
  start_linux_pool_server(argv[2], argc >= 4 ? argv[3] : NULL,
                          argc >= 5 ? argv[4] : NULL);
