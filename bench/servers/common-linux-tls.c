...
static int send_exactly(struct conn *conn, void *buf, size_t size) {
  ...
  while (partial < size) {
    ret = send(conn->fd, &cbuf[partial], size - partial, MSG_NOSIGNAL);    if (ret <= 0)
      return ret;
    partial += ret;
  }
...
static void memcached_drive_machine(struct conn *conn){
  ...
  next_request:
    ...
    if (handle_ret(conn, ret, __LINE__))
      return;
    printf("starting to state_header%d\n", conn->header.bin_header.magic);
  ...
  while (1) {
    ...
    for (i = 0; i < nfds; i++) {
      if (events[i].data.u32 == 0) {
        ...
        // if (setsockopt(conn_sock, SOL_TCP, TCP_ULP, "tls", sizeof("tls"))) {
        ...
        // // Set the socket options to enable KTLS
        // if (setsockopt(conn_sock, SOL_TLS, TLS_RX, &crypto_info, sizeof(crypto_info)))
        // {
        //   perror("setsockopt TLS_RX");
        //   return 1;
        // }
        // if (setsockopt(conn_sock, SOL_TLS, TLS_TX, &crypto_info, sizeof(crypto_info)))
        // {
