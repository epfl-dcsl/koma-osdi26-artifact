...
// TODO: I should be able to include libix/syscall.h but instead I must do the
// following upto MARKER
#define SYSCALL_START  0x100000
#define SYS_SPAWNMODE 5
...
{
	return __syscall_1(SYS_SPAWNMODE, spawn_cores);
}
// MARKER
...
public:
      ...
      warehouse_id_end(warehouse_id_end)
  {
    ...
    cerr << "warehouse start " << warehouse_id_start << " numwhs " << NumWarehouses() << endl;
...
tpcc_worker::txn_new_order()
{
  ...
  try {
    ...
    if (likely(db->commit_txn(txn))){
      //cerr << "[TXN New_Order]: End success" << endl;
      return txn_result(true, ret);
    }
  } catch (abstract_db::abstract_abort_exception &ex) {
    db->abort_txn(txn);
  }
  //cerr << "[TXN New_Order]: End failure" << endl;
  ...
  try {
    ...
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      ...
      if (unlikely(!tbl_oorder(warehouse_id)->get(txn, Encode(obj_key0, k_oo), obj_v))) {
        ...
        db->abort_txn(txn);
        //cerr << "[TXN Delivery]: End failure" << endl;
        return txn_result(false, 0);
      }
    ...
    if (likely(db->commit_txn(txn))) {
      //cerr << "[TXN Delivery]: End success" << endl;
      return txn_result(true, ret);
    }
  } catch (abstract_db::abstract_abort_exception &ex) {
    db->abort_txn(txn);
...
tpcc_worker::txn_payment()
{
  ...
  try {
    ...
    if (likely(db->commit_txn(txn))) {
      //cerr << "[TXN Payment]: End success" << endl;
      return txn_result(true, ret);
    }
  } catch (abstract_db::abstract_abort_exception &ex) {
    db->abort_txn(txn);
  }
  //cerr << "[TXN Payment]: End failure" << endl;
  return txn_result(false, 0);
}
...
{
  //cerr << "[TXN Order_Status]: Begin" << endl;
  const uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
  const uint districtID = RandomNumber(r, 1, NumDistrictsPerWarehouse());
  ...
  try {
    ...
    if (likely(db->commit_txn(txn))) {
      //cerr << "[TXN Order_Status]: End success" << endl;    
      return txn_result(true, 0);
    }
  } catch (abstract_db::abstract_abort_exception &ex) {
    db->abort_txn(txn);
  }
  //cerr << "[TXN Order_Status]: End failure" << endl;
  return txn_result(false, 0);
}
...
{
  //cerr << "[TXN Stock_Level]: Begin"<< endl;
  const uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
  ...
  try {
    ...
    if (likely(db->commit_txn(txn))){
      //cerr << "[TXN Stock_Level]: End success" << endl;
      return txn_result(true, 0);
    }
  } catch (abstract_db::abstract_abort_exception &ex) {
    db->abort_txn(txn);
  }
  //cerr << "[TXN Stock_Level]: End failure"<< endl;
  return txn_result(false, 0);
}
...
/*XXX(aghosn) Actual code for ix.*/
class tpcc_ix_worker : public tpcc_worker
{
public:
  tpcc_ix_worker( unsigned int worker_id,
                  unsigned long seed,
                  abstract_db *db,
                  const map<string, abstract_ordered_index *> &open_tables,
                  const map<string, vector<abstract_ordered_index *>> &partitions,
                  spin_barrier *barrier_a, spin_barrier *barrier_b,
                  uint warehouse_id_start, uint warehouse_id_end)
  : tpcc_worker(worker_id, seed, db, open_tables, partitions,
    barrier_a, barrier_b,
    warehouse_id_start, warehouse_id_end)
  {}

  void proxy_exec(int trans) {
    //scoped_db_thread_ctx ctx(db, false);
    
    /*  TODO not sure if these are needed or not*/
    //txn_counts.resize(workload.size());
    //barrier_a->count_down();
    //barrier_b->wait_for();
    
    timer t;
    const auto ret = workload[trans].fn(this);
    if (likely(ret.first)) {
      ++ntxn_commits;
    ...
    txn_counts[trans]++; // txn_counts aren't used to compute throughput (is
                     // just an informative number to print to the console
                     // in verbose mode)

    //db->do_txn_finish(); from bench_runner::run???
  }
  ...
  void proxy_exec_rd() {
    ...
    for (unsigned int i = 0; i < workload.size(); i++) {
      double upper = threshold + workload[i].frequency; 
      if (d >= threshold && d < upper) {
        proxy_exec(i);
        return;
      }

      threshold += workload[i].frequency;
    }
  }

  size_t size_workload() {
    return this->get_workload().size();
  }
...
class tpcc_ix_runner : public tpcc_bench_runner
{
public:
  tpcc_ix_runner(abstract_db *db) : tpcc_bench_runner(db)
  {}

  vector<bench_worker *> get_workers()
  {
    return this->make_workers();
  }

  vector<bench_loader*> get_loaders()
  {
    return this->make_loaders();
  }

  map<string, abstract_ordered_index*> get_open_tables()
  {
...
void dcsl_create_runner_partitions( abstract_db *db,
                                    bench_runner* &runner,
              std::map<std::string, abstract_ordered_index *> &open_tables,
              std::map<string, vector<abstract_ordered_index *>> &partitions) {
  
  //runner = new tpcc_bench_runner(db);
  runner = new tpcc_ix_runner(db);
  partitions = static_cast<tpcc_ix_runner*>(runner)->partitions;
  open_tables = static_cast<tpcc_ix_runner*>(runner)->get_open_tables();
}


/*Code from make_loaders and bench_runner::run*/
//void dcsl_init_mloaders(abstract_db *db,
...
void dcsl_init_mloaders(bench_runner *runner, vector<bench_loader*> &ret) {
  ret = static_cast<tpcc_ix_runner*>(runner)->get_loaders();
}

void dcsl_init_mworkers(std::vector<bench_worker *> &workers, bench_runner *runner)
{
  workers = static_cast<tpcc_ix_runner*>(runner)->get_workers();
  cerr <<"WORKERS size " << workers.size() << endl; 
  cerr <<"WORKERS front workload size " << static_cast<tpcc_ix_worker *>(workers.front())->size_workload() << endl;
}

void dcsl_exec(int worker_id, int nb, std::vector<bench_worker *> *workers)
{
  static_cast<tpcc_ix_worker *>(workers->at(worker_id))->proxy_exec(nb);
}

void dcsl_exec_rd(int worker_id, std::vector<bench_worker *> *workers)
{
  static_cast<tpcc_ix_worker *>(workers->at(worker_id))->proxy_exec_rd();
}

void __dcsl_init_worker(int worker_id, std::vector<bench_worker *> *workers)
{
  static_cast<tpcc_ix_worker *>(workers->at(worker_id))->dcsl_init();
}
